...
def _setup_apply_run(tmp_path):
    ...
    (run_dir / "patch.diff").write_text(patch_content, newline="")
    run_meta = {
        "run_id": "test-run",
        "status": "previewed",
...
def test_apply_rollback_block1(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 1
    assert "candidate commit failed" in result.stdout
    assert "FATAL" not in result.stdout


def test_candidate_validation_rejection_leaves_checkout_unchanged(tmp_path, monkeypatch):
    """A rejected candidate is never promoted or rolled back in the caller's checkout."""
    repo, ws, _ = _setup_apply_run(tmp_path)

    monkeypatch.setattr(
        "orchestrator.agents.validator.run",
        lambda *a, **kw: (ValidatorOutput(overall_passed=False), {}),
    )
    ...
    assert result.exit_code == 1
    assert "candidate validation was not authorized" in result.stdout
    assert (repo / "f.txt").read_text() == "hello\n"
    assert "FATAL" not in result.stdout
