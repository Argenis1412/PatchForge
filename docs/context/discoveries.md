...
### ✅ [2026-07-22] Dogfooding 011 — gemini-2.5-flash fallback silently degrades executor for medium-sized files (D-011d) (RESOLVED, mitigation 2 — Parts 1-3 of 3)
...
- **Resolution (Part 2 — issue #278, branch `fix/issue-278-executor-fallback-warning`):** Addresses mitigation (2) for the **executor** path (`preview`, `pipeline.py`). `FileChange` gained `primary_provider_attempted`/`primary_failure_category` (both `str | None = None`, backward-compatible with pre-existing persisted `executor_<run_id>.json` files). New shared helper `agents/executor/fallback.py` (`collect_fallback_changes`/`log_fallback_events`) is the single source of truth for "which `FileChange`s count as a fallback" and "log the event for each," reused verbatim by `preview.py` (console print + event) and `pipeline.py`'s `_stage_executor` (event only — no `Console` instance exists in `pipeline.py`). The fallback filter requires **both** `status in {APPLIED, NOOP, PENDING_REVIEW}` **and** `provider_name != primary_provider_attempted` — the status restriction, added during an external `/adversarial` pass, is load-bearing: a fallback provider's response can still fail Python syntax validation and be discarded (`status=ERROR`, `provider_name` still set), and without it the warning would misreport a terminal failure as "fell back, now using X." `pipeline.py`'s `--from-stage executor` resume branch deliberately does **not** re-call `log_fallback_events` — an earlier draft of this plan added it there to avoid "losing" the event on resume, which a later adversarial pass identified as backwards: the event was already emitted once during the original fresh run, and re-emitting on resume would duplicate telemetry under a new `trace_id` and mislabel a skipped stage as active executor work. `pipeline.py`'s `TaskResult.model_used` was also fixed in the same PR (`change.provider_name or "n/a"`, never the aggregate `"GM:...|OR:...|CL:..."` config string it used unconditionally before, and never a mix of provider-shortname and aggregate-string shapes as an early draft of the fix did) — a real audit/provenance gap surfaced by an external adversarial pass, cheap to fix since the same loop and the same `provider_name` data were already being touched. Five `/adversarial` passes total across this issue's planning (two self-review rounds via the `/adversarial` skill, three further external reviews pasted into the session, numbered attacks 1-7 across them) — see items below for what was raised and deliberately rejected as out of scope. A subsequent `/diff-review` pass caught one real bug the adversarial rounds missed: `preview.py`'s fallback warning escaped `primary_provider_attempted`/`primary_failure_category`/`provider_name` via `rich.markup.escape()` but not `change.file` on the same line — a file path containing literal brackets could raise `rich.errors.MarkupError` or corrupt the line; fixed, and the existing Rich-escaping test extended to cover a bracketed file name too. The same pass added a `noop`-outcome fallback test (`_apply_task` threading the two new fields on an idempotent fallback), which had been missed even though the combination is reachable in production. Tests: `tests/test_executor.py` (`_apply_task` field-threading + `collect_fallback_changes` filtering, 15 tests), `tests/test_preview_execute.py` (4 tests: warning text, no-false-positive, syntax-error exclusion, Rich markup escaping incl. bracketed file names), `tests/test_pipeline.py` (6 tests: event emission, no-event on no-fallback, no-event on persist failure, no-re-emit on resume, `TaskResult.model_used` accuracy, OSError-tolerance). QA green (988 passed, 6 skipped).
- **Resolution (Part 3 — issue #280, branch `fix/issue-280-ci-provider-fallback-visibility`):** Confirmed by reading the code that `ci.py` calls the agent-level functions directly (`architect_agent.run()`/`run_from_issue()`, `executor_agent.run()`), bypassing the command layer entirely — so, unlike the hypothesis raised when Part 2 closed, `ci.py` does **not** reimplement the pipeline steps inline; it simply never inspected the fallback data (`arch_meta`, `executor_output`) that Parts 1-2 already thread through, so no `provider_fallback` event ever reached `pipeline.jsonl` for CI runs.

  An external `/adversarial` pass on the initial `ci.py`-only draft raised three attacks, all accepted, expanding the fix into a small cross-cutting cleanup rather than a `ci.py`-only patch:
  1. **DRY (architect side had no shared helper, unlike executor's `agents/executor/fallback.py`):** new `agents/architect/fallback.py` (`detect_fallback`/`log_architect_fallback`, mirroring the executor module's split between pure detection and I/O-tolerant event emission) is now the single source of truth for the architect's `arch_meta` fallback check. `commands/plan.py` was migrated to use it too (behavior-preserving — same console text, same event shape, existing `test_v1_commands.py` assertions pass unchanged), closing the duplication at its root instead of adding a third divergent copy in `ci.py`.
  2. **Severity mismatch (architect fallback logged `level="warning"`, executor fallback logged `level="info"` by omission — backwards, since executor-stage fallback is the more operationally critical one):** `agents/executor/fallback.py::log_fallback_events` gained a `level: str = "warning"` parameter, retroactively fixing the severity for all three of its callers (`preview.py`, `pipeline.py`, and now `ci.py`) with no call-site changes needed. Confirmed via grep that no existing test asserted on `level`, so the default bump was safe.
  3. **`source` conflation (the initial draft forced `source="pipeline"` on `ci.py`'s architect event to match `plan.py`, but `ci.py` uses `source="ci"` for every other event it emits):** `ci.py` now passes `source="ci"` for its architect fallback event, consistent with its own convention; `plan.py` keeps `source="pipeline"`. `event="provider_fallback"` remains the intended cross-entrypoint aggregation key, not `source` — a consumer aggregating fallbacks across `plan.py`/`preview.py`/`pipeline.py`/`ci.py` should filter by event name, not by a forced-uniform `source`.

  A second `/adversarial` pass on the original `ci.py`-only draft (before the three attacks above) also caught a critical placement bug: the fallback-check blocks were originally drafted to sit *inside* the existing `try`/`except Exception` blocks wrapping the architect/executor agent calls in `ci.py` — meaning a bug in the new fallback-logging code itself would have been mis-caught by those handlers and misreported as `"Architect failed"`/`"Executor failed"` for a run whose agent stage actually succeeded. Fixed by placing both blocks strictly after their respective `try/except` closes (confirmed exact line anchors by reading the file directly), mirroring where Part 1/Part 2 place the equivalent checks in `plan.py`/`preview.py` relative to the same gates (`check_plan_gate`, `check_patch_gate`).

  New files: `agents/architect/fallback.py`. Modified: `agents/architect/__init__.py` (re-export), `commands/plan.py` (migrated to the shared helper), `agents/executor/fallback.py` (`level` param), `commands/ci.py` (both fallback blocks). Tests: `tests/test_architect.py` (`TestArchitectFallbackHelper`, 5 tests for `detect_fallback`/`log_architect_fallback` incl. OSError tolerance), `tests/test_executor.py` (1 test for `log_fallback_events`'s new `level` default), `tests/test_ci_command.py` (`TestCiProviderFallback`, 7 tests: architect fallback event+warning, architect no-false-positive, architect `primary_provider_attempted is None` no-op, architect OSError tolerance, executor fallback event+warning, executor no-false-positive, executor syntax-error-after-fallback exclusion). QA green (1002 passed, 6 skipped).

  Deferred, not addressed by this part (pre-existing or explicitly out of scope): the `CiResult`/`RunMetadata` schema gap (no structured fallback field on either, same category as the pre-existing gap noted for Part 2); the architect-call-exception case (if `architect_agent.run()`/`run_from_issue()` raises before returning `arch_meta`, no fallback event is emitted for that call — identical, pre-existing behavior to Part 1, not a regression).
...
### [2026-07-22] Issue #278 — three test-coverage gaps flagged by `/diff-review` left un-expanded to avoid growing this issue's scope
