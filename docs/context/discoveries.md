...
### [2026-07-18] `apply.execute()` has a redundant HEAD-divergence check that makes the REBASEABLE branch unreachable
...
- **Why deferred:** Fixing this means picking one of two designs (rely on `classify_lifecycle` + drop the early check, or keep the early check + drop the `REBASEABLE` branch) and is unrelated to #258's scope (ALREADY_APPLIED detection). Revisit alongside a future `apply.py` control-flow cleanup rather than folding it into an unrelated fix.
